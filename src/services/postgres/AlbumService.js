...
class AlbumsService {
  ...
  async getAlbumById(id) {
    ...
    return {
      ...resultAlbum.rows.map((album) => ({
        id: album.id,
        name: album.name,
        year: album.year,
        coverUrl: album.cover ? `http://${process.env.HOST}:${process.env.PORT}/public/cover/${album.cover}` : null,
      }))[0],
      songs: resultSongs.rows.map(mapSongDBToModel),
    };
  }
